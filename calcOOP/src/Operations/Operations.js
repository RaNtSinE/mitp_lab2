class Operations {
    ...
    constructor() {
        ...
        this.merger = new OperationMerge();
    }
    ...
    doMath(expressions) {
        ...
        while (!exit)
        {
            ...
            if (targetExpression !== -1)
            {
                ...
                while (firstPriorityOperators > 0)
                {
                    for (let j = 0; j < expression.length; j++)
                    {
                        ...
                        {
                            let numbers = this.parser.getNumbers(expression, j);
                            let result = 0;

                            if (expression[j] === "/")
                                result = this.division.doOperation(numbers[0], numbers[1]);
                            else
                                result = this.multiply.doOperation(numbers[0], numbers[1]);
                ...
                while (secondPriorityOperators > 0)
                {
                    for (let j = 1; j < expression.length; j++)
                    {
                        ...
                        {
                            let numbers = this.parser.getNumbers(expression, j);
                            result = this.addition.doOperation(numbers[0], numbers[1]);
                ...
                expressions = this.merger.doMerge(expressions, targetExpression)
            ...
            // expression.push(this.parser.parseOperations(expressions[i].str))
